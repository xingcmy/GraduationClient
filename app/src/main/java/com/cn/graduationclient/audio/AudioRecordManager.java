...
public class AudioRecordManager implements Handler.Callback {
    ...
    public static AudioRecordManager getInstance(Context context,String UID,String id,String time) {
        if (mInstance == null) {
            synchronized (AudioRecordManager.class) {
                if (mInstance == null) {
                    mInstance = new AudioRecordManager(context,UID,id,time);
                }
            }
        }
        return mInstance;
    ...
    @TargetApi(21)
    private AudioRecordManager(Context context,String UID,String id,String time) {
        this.UID=UID;
        this.id=id;
        this.time=time;
    ...
    public void setAudioSavePath(String path) {
        if (TextUtils.isEmpty(path)) {
            this.SAVE_PATH = mContext.getExternalFilesDir(null).getPath();
        } else {
            this.SAVE_PATH = path;
        }
    }
...
//                int bps = e.getInteger(e.getIdentifier("rc_audio_encoding_bit_rate", "integer", this.mContext.getPackageName()));
            ...
            //存储
            this.mAudioPath = Uri.fromFile(new File(SAVE_PATH,UID+"to"+id+time+ ".voice"));
